...
export function convertTime(seconds: string | number, forceHours: boolean = true) {
  seconds = seconds.toString();
  let totalMinutes = Math.floor(Number(seconds) / 60);
  let hours = Math.floor(totalMinutes / 60).toString();
  let minutes = (totalMinutes % 60).toString();
  let remainingSeconds = Math.floor(Number(seconds) % 60).toString();

  hours = Number(hours).toString();
  minutes = Number(minutes) >= 10 ? minutes : `0${minutes}`;
  remainingSeconds = Number(remainingSeconds) >= 10 ? remainingSeconds : `0${remainingSeconds}`;

  if (forceHours || Number(hours) > 0) {
